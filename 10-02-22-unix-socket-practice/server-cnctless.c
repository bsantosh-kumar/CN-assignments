...
int bindConctLessUSServer(char *pathName) //bind connection less unix socket server
{
    int usfd;
    struct sockaddr_un userv_addr;
    int userv_len;

    if ((usfd = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0)
    {
...
int main()
{
    int usfd = bindConctLessUSServer("first-unix-socket-cont");
    int nusfd;
    ...
    while (1)
    {
        ...
        if (recvfrom(usfd, buff, 128, 0, (struct sockaddr *)&ucli_addr, (socklen_t *)&ucli_len) < 0)
        {
        ...
        bzero(buff, sizeof(buff));
        strcpy(buff, "Hi from server");
        if (sendto(usfd, buff, 128, 0, (struct sockaddr *)&ucli_addr, ucli_len) < 0)
        {
